...
<head>
 ...
 <script>
    function validateCaptcha() {
      var userCaptcha = document.getElementById('captcha').value.toLowerCase();
      var serverCaptcha = document.querySelector('.captcha').textContent.toLowerCase();
    ...
    <main class="form-signin w-100 m-auto shadow rounded-top-3">
      <form method="post" onsubmit="return validateCaptcha();">
        ...
        <div class="mb-2 form-floating">
          <input value="<?= old_value('email') ?>" name="email" type="email" class="form-control" id="floatingInput" placeholder="name@example.com" required>
          <label for="floatingInput">Email address</label>
        </div>
        <div class="mb-2 form-floating">
          <input value="<?= old_value('password') ?>" name="password" type="password" class="form-control" id="floatingPassword" placeholder="Password" required>
          <label for="floatingPassword">Password</label>
        </div>

        <div class="col-md-10 form-group">
          <label for="captcha" class="ms-2 mb-1">Captcha</label>
          <div class="d-flex">
            <input type="text" name="captcha" id="captcha" placeholder="Enter Captcha" class="form-control" minlength="4" required />
            <input type="hidden" name="captcha-rand" value="<?php echo $rand; ?>">
            <div class="captcha ms-3"><?php echo $rand; ?></div>
          </div>
        </div>
        ...
        <div class="text-start my-3 center">
          <p class="unselectable mb-0">Don't have an account? <a id="link" href="<?= ROOT ?>/signup">Sign up</a></p>
        </div>
        <hr>
        <div class="text-start my-3 center">
          <p class="unselectable mb-0">For Administrator? <a id="link" href="<?= ROOT ?>/admin-login">Log In</a></p>
        </div>
        <hr>
